...
class CommandParser:
    ...
    @staticmethod
    def _extract_med_add(text: str) -> str:
        text = clean_spoken_text(text)
        text = re.sub(
    ...
    @staticmethod
    def _extract_med_remove(text: str) -> str:
        text = clean_spoken_text(text)
        text = re.sub(
    ...
    def process(self, text: str) -> str:
        ...
        for category, phrases in category_map.items():
        ...
        # Medicamentos.
        if any(v in t for v in ["agregar", "agrega", "anadir", "registrar", "programar"]) \
                and any(n in t for n in ["medicamento", "medicina", "pastilla"]):
            time_value = parse_time(original)
            name = self._extract_med_add(original)
            if not name:
        ...
        if any(x in t for x in [
            ...
            name = self._extract_med_remove(original)
        ...
        if any(x in t for x in [
            "quitar contacto", "eliminar contacto"
        ]):
            name = re.sub(
        ...
        if any(x in t for x in [
            "mis contactos", "lista de contactos", "que contactos"
        ]):
        ...
        if any(x in t for x in ["ayuda", "comandos", "que puedo hacer"]):
            return (
                ...
                "noticias de economía; buenas noticias; emergencia; o qué hora es."
            )
