...
class AlarmManager:
    ...
    @staticmethod
    def alarm_id(med_id: int, chat_id: int) -> str:
        return f"med_{med_id}_{chat_id}"

    @staticmethod
    def reminder_id(med_id: int, chat_id: int) -> str:
        return f"rec_{med_id}_{chat_id}"

    def cancel_reminders(self, med_id: int) -> None:
        for chat_id in self.settings.usuarios_alarma:
            try:
                self.scheduler.remove_job(
                    self.reminder_id(med_id, chat_id)
                )
            except Exception:
                pass
        ...
        for med in self.meds.list():
            ...
            for chat_id in self.settings.usuarios_alarma:
                self.scheduler.add_job(
                    ...
                    args=[med["id"], chat_id],
                    id=self.alarm_id(med["id"], chat_id),
    ...
    def fire(self, med_id: int, chat_id: int) -> None:
        ...
        self.scheduler.add_job(
            ...
            args=[med_id],
            id=self.reminder_id(med_id, chat_id),
            replace_existing=True,
        )
